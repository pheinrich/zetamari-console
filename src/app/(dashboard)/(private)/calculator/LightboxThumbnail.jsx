...
// (see the request this was built from) - no toolbar, no dimensions -
// just the image, a selected-state border, a Close button that only
// appears on hover, and (since the working panel got an editable label)
// the entry's label as a hover tooltip. Resolves its own geometry from
// the entry's stored outsideId/insideId/rabbetId/width/height/border
// rather than depending on whatever the main working panel currently
// shows, so old entries keep rendering correctly even after the main
// panel moves on.
export default function LightboxThumbnail( {entry, contours, selected, onSelect, onRemove, dragHandleProps} )
{
  const [hovering, setHovering] = useState( false )

  const mirror = useMemo(
    () => resolveEntryMirror( entry, contours ),
    [entry, contours]
  )
  ...
  return (
    <Tooltip title={labelForEntry( entry )}>
      <div
        {...dragHandleProps}
